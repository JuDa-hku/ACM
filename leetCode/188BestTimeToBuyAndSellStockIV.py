...
#        print newEarning
        cache = {}
        beginIndex = 0
        maxProfit = self.partialSum(k, newEarning, beginIndex, cache)
        return maxProfit
        

    def partialSum(self,k,earnings,beginIndex, cache):
        if (k, beginIndex) in cache:
            print 1
            return cache[(k, beginIndex)]
        tmp = earnings[beginIndex:]
        positiveIndex = [index for index in range(len(tmp)) if tmp[index]>0]
        if k>=len(positiveIndex):
             cache[(k, beginIndex)]=sum([tmp[index] for index in positiveIndex])
             return cache[(k,beginIndex)]
        if len(positiveIndex) == len(tmp):
            cache[(k,beginIndex)] = sum(tmp)
            return sum(tmp)
        if k == 1:
            cache[(k,beginIndex)] = self.partialMaxSum(earnings[beginIndex:])
            return cache[(k,beginIndex)]
        else:
            maxProfit = 0
            for index in range(beginIndex, len(earnings)):
                if earnings[index]<0:
                    cutIndex = index
                    nextBeginIndex = cutIndex + 1
                    WithPartOne = self.partialMaxSum(earnings[beginIndex:cutIndex]) + self.partialSum(k-1, earnings, nextBeginIndex, cache)
                    WithOutPartOne = self.partialSum(k, earnings, nextBeginIndex, cache)
                    maxCut = max(WithPartOne, WithOutPartOne)
                    maxProfit = max(maxProfit, maxCut)
                    cache[k,beginIndex] = maxProfit
            return maxProfit

    def partialMaxSum(self, earnings):
        maximum = 0
        start = 0
...
k = 29
prices = [70,4,83,56,94,72,78,43,2,86,65,100,94,56,41,66,3,33,10,3,45,94,15,12,78,60,58,0,58,15,21,7,11,41,12,96,83,77,47,62,27,19,40,63,30,4,77,52,17,57,21,66,63,29,51,40,37,6,44,42,92,16,64,33,31,51,36,0,29,95,92,35,66,91,19,21,100,95,40,61,15,83,31,55,59,84,21,99,45,64,90,25,40,6,41,5,25,52,59,61,51,37,92,90,20,20,96,66,79,28,83,60,91,30,52,55,1,99,8,68,14,84,59,5,34,93,25,10,93,21,35,66,88,20,97,25,63,80,20,86,33,53,43,86,53,55,61,77,9,2,56,78,43,19,68,69,49,1,6,5,82,46,24,33,85,24]
print s.maxProfit(k, prices)